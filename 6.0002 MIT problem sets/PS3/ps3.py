...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.width = width
        self.height = height
        self.tiles = {(w, h): dirt_amount for w in range(0, self.width) for h in range(0, self.height)}
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        self.tiles[(math.floor(pos.get_x()), math.floor(pos.get_y()))] -=\
            capacity if capacity <= self.tiles[(math.floor(pos.get_x()), math.floor(pos.get_y()))] else 0
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        return len([n for n in self.tiles.values() if n == 0])
    ...
    def is_position_in_room(self, pos):
        ...
        """
        return\
            (pos.get_x()  > 0 and pos.get_x() < self.width)\
...
class Robot(object):
    ...
    def get_robot_speed(self):
        """
        Not sure if this is necessary, keeps the structure constant though
        """
        return self.speed
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        # return Position(random.seed(1) * self.width, random.seed(1) * self.height)
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        # Constant pos for testing
        # pos = Position(random.seed(4) * self.width, random.seed(4) * self.height)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # if (self.room.cl
        newPosition = self.position.get_new_position(self.get_robot_direction(), self.get_robot_speed())
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        newPosition = self.position.get_new_position (self.get_robot_direction (), self.get_robot_speed ())
        # print type(newPosition)
        if (self.room.is_position_valid (newPosition) and not self.gets_faulty()):
            self.set_robot_position (newPosition)
            self.room.clean_tile_at_position (newPosition, self.capacity)
        else:
            self.set_robot_direction (random.random () * 360)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(0, num_trials):
        ...
        # To animate
        # animate = ps3_visualize.RobotVisualization (num_robots, width, height, False, 0.000001)
        robots = [robot_type (room, speed, capacity) for i in range(0, num_robots)]
        while coverage < min_coverage:
            time_steps += 1
            for robot in robots:
                robot.update_position_and_clean()
                # animate.update (room, robots)
...
#       of a 20x20 room?
# Standard does better early on but with addition of multiple robots the performance conjoins(if that's even a word)
# kinda like how companies hire far too many lackluster employees(like me),
# so that they can cover up for their lack of talent
# Law of large numbers basically.
#
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
# There is a slight drop in performance for standard robot after a slight increase, probably because the angles that
# lead to successful/efficient outcomes are far less when walls are much closer(high aspect ratio)
# This turns out considerably worse for faulty robot, on the off chance it's on the right path,
# it might end up faulting and go on the wrong/worse path or repeat the previous path
#
# Platitudes, I say platitudes. My answers are all platitudes
