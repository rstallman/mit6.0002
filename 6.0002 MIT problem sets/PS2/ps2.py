...
# represented?
#
# Answer: Nodes are intersections, edges are roads in the map. Total distance and outdoor distances are 3rd and 4th column respectively
#
    ...
    if (not digraph.has_node(startNode)) or (not digraph.has_node(endNode)):
        ...
        for edge in digraph.edges[startNode]:
            ...
            if path[0].count(edge.get_destination().get_name()) < 1 and newPath[1] <= best_dist and newPath[2] <= max_dist_outdoors:
                ...
                traversed_path = get_best_path(digraph, edge.get_destination().get_name(), end, newPath, max_dist_outdoors, best_dist, best_path)
                if type(traversed_path) != str:
                    if traversed_path[1] < best_dist:
                        best_dist = traversed_path[1]
                        best_path = traversed_path[0]
                newPath = [path[0][:], path[1], path[2]]
    if len(best_path) == 0:
        return "Failed"
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    """
    bestPath = get_best_path(digraph, start, end,[[start], 0, 0], max_dist_outdoors, max_total_dist, [])
    if bestPath[1] > max_total_dist:
        raise ValueError('Exceed the maximum allowable total distance')
