...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return self.src.name + '->' + self.dest.name + ' ' + str((self.total_distance, self.outdoor_distance))
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        """Adds an Edge or WeightedEdge  to the Digrainstanceph. Raises a
        ValueError if either of the nodes associated with the edge is not
        in the  graph."""
        if self.has_node(edge.src) != True or self.has_node(edge.dest) != True:
            raise ValueError('Value not in graph')
        # print(type(edge.src), [type(node) for node in self.nodes])
        if self.edges.has_key(edge.src):
            self.edges[edge.src].append(edge)
        else:
            self.edges.update({edge.src: [edge]})
